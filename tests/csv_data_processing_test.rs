...
,Afghanistan,33.93911,67.709953,0,0,0,0"
    ...
    let result = data_processing::process_csv(
        ...
        global_deaths_csv,
        Region::Global,
        "1/26/20".to_string().to_string(),
        Some(global_confirmed_and_deaths_count),
    );

    let (actual_global_cases, actual_time_series_map) = match result {
        Ok((actual_global_cases, actual_time_series_map)) => {
            (actual_global_cases, actual_time_series_map)
        }
        Err(_) => panic!("Error extracting data from global time series."),
    };
